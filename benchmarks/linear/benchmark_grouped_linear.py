...
def build_grouped_linear_case(m, k, n, num_gemms, m_splits, bias):
    device = "cuda"
...
def profile_linear(layer, x, m_splits, mode, gradient, recipe_name, warmup_steps, profile_steps):
    if profile_steps < 1:
    ...
    if warmup_steps > 0:
        ...
        run_linear_multiple_steps(layer, x, m_splits, mode, gradient, warmup_steps, recipe)
        torch.cuda.nvtx.range_pop()
    ...
    start_event.record()
    run_linear_multiple_steps(layer, x, m_splits, mode, gradient, profile_steps, recipe)
    end_event.record()
...
def run_benchmark_linear(
    ...
    metric_name = "grouped_fwd_time_ms" if fwd_only else "grouped_fwd_bwd_time_ms"
    if profile:
        metric_name = "grouped_profile_step_time_ms"

    print(f"========== Benchmarking {recipe_name} ==========")
    for m, k, n in mkns:
        m_splits = [m // num_gemms] * num_gemms if m_splits_provided is None else m_splits_provided
        layer, x, gradient = build_grouped_linear_case(
            m,
            k,
            n,
            num_gemms=num_gemms,
        ...
        data.append([m, k, n, recipe_name, num_gemms, timing_ms])

    df = pd.DataFrame(
        data=data,
        columns=["m", "k", "n", "recipe", "num_gemms", metric_name],
    )
...
def output_csv_path(output_dir, output_file, profile, recipe, fwd_only):
    ...
    return out_dir / f"benchmark_grouped_linear_{run_kind}_{recipe}_{mode}.csv"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Jagged input to use, example:"
            " 15296,8960,14656,14784,11712,7936,14080,10880"
        ),
    )
    parser.add_argument("--hidden-dim", type=int, default=7168, help="Hidden dimension to use")
    parser.add_argument("--output-dim", type=int, default=2048, help="Output dimension to use")
    parser.add_argument(
        "--fwd-only",
        action="store_true",
        default=False,
    ...
    if args.profile:
        num_gemms_list = [8]
        hidden_dim_to_profile = args.hidden_dim
        output_dim_to_profile = args.output_dim
