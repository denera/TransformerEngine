...
class GemmPrimitive(BasePrimitive):
    ...
    name = "te_gemm"
    impl_static_args = (7, 8, 9, 10, 11, 12, 13)
    ...
    def partition(out_dtype, contracting_dims, do_gelu, use_bias, grad, accumulate,
        ...
        out_spec = [*batch_specs, lhs_outer_spec, rhs_outer_spec]
        out_sharding = NamedSharding(mesh, PartitionSpec(*out_spec))
        gelu_sharding = NamedSharding(mesh, PartitionSpec(out_spec if do_gelu else None))
        ...
        def sharded_impl(lhs, lhs_scale_inv, rhs, rhs_scale_inv, bias, out_amax, out_scale):
            output, out_amax, out_scale, pre_gelu_out = GemmPrimitive.impl(
                lhs,
                lhs_scale_inv,
                rhs,
                rhs_scale_inv,
            ...
            if rhs_spec[rhs_inner_dim] is not None:
                ...
                # If the outer dimension of the LHS is sharded, we also need to scatter the output,
                # which we combine here into a reduce-scatter
                par_op = jax.lax.psum if lhs_outer_spec is None else jax.lax.psum_scatter
                output = lax_paral_op(output, par_op, global_mesh_resource().tp_resource, mesh)
                if do_gelu:
                    pre_gelu_out = lax_paral_op(
                        pre_gelu_out, par_op, global_mesh_resource().tp_resource, mesh
                    )
            elif lhs_outer_spec is None:
                # If the outer dimension of LHS is sharded, we need to scatter the output.
                output = jax.lax.with_sharding_constraint(output, out_sharding)
                if do_gelu:
                    pre_gelu_out = jax.lax.with_sharding_constraint(pre_gelu_out, gelu_sharding)
