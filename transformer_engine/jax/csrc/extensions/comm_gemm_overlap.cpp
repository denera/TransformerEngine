...
void BootstrapCommGemmOverlap(
    const std::string &overlap_name, const std::string &method,
    const std::vector<size_t> &buffer_shape, DType buffer_dtype, CommOverlapType comm_type,
  ...
  NVTE_CHECK(_overlaps.find(overlap_name) == _overlaps.end(), overlap_name,
             " is already initialized!");
  if (method == "ring-exchange") {
    _overlaps[overlap_name] = reinterpret_cast<CommOverlapCore *>(
      new CommOverlapP2PBase(buffer_shape, buffer_dtype, -1, -1, -1, -1, -1, -1, tp_size,
                             &_dummy_allgather, &_dummy_barrier, comm_type, num_max_streams,
                             comm_cga_size, num_comm_sm, set_sm_margin, use_ce, atomic_gemm,
                             aggregate)
    );
  } else {
    _overlaps[overlap_name] = reinterpret_cast<CommOverlapCore *>(
      new CommOverlapBase(buffer_shape, buffer_dtype, -1, -1, -1, -1, -1, -1, tp_size,
                          &_dummy_allgather, &_dummy_barrier, num_splits, num_max_streams,
...
void DestroyCommGemmOverlap() {
  for (auto it = _overlaps.begin(); it != _overlaps.end();) {
    delete it->second;
    _overlaps.erase(it++);
  }
};

template <typename CommOverlapClass>
void SetOverlapBufferScaleInverse(const std::string &name, pybind11::object scale_inv) {
  auto scale_inv_tensor = DLPackWrapper(scale_inv);
  reinterpret_cast<CommOverlapClass *>(_overlaps[name])->set_ubuf_scale_inv(
      reinterpret_cast<float *>(scale_inv_tensor.dptr()));
}

template <typename CommOverlapClass>
pybind11::object GetOverlapBuffer(const std::string &name, CommOverlapType comm_type) {
  auto output = reinterpret_cast<DLPackWrapper *>(
    &(reinterpret_cast<CommOverlapClass *>(_overlaps[name])->get_ubuf_output(comm_type))
  );
  return output->capsule();
};

void CopyIntoOverlapBufferImpl(
    cudaStream_t stream, void *input_ptr, const std::vector<size_t> &shape,  DType dtype,
    const std::string &name, CommOverlapType comm_type) {
  auto input = TensorWrapper(input_ptr, shape, dtype);
  if (_overlaps[name]->is_p2p_overlap()) {
    reinterpret_cast<CommOverlapP2PBase *>(_overlaps[name])->copy_into_ubuf(
        stream, input, comm_type);
  } else {
    reinterpret_cast<CommOverlapBase *>(_overlaps[name])->copy_into_ubuf(stream, input, comm_type);
  }
...
void CopyIntoOverlapBuffer(cudaStream_t stream, void **buffers, const char *opaque,
  ...
  CopyIntoOverlapBufferImpl(
      stream, input_ptr, std::vector<size_t>(desc.shape, desc.shape+desc.ndim), desc.dtype,
...
Error_Type CopyIntoOverlapBufferFFI(cudaStream_t stream, Buffer_Type input, std::string_view name,
  ...
  CopyIntoOverlapBufferImpl(stream, input_ptr, shape, dtype, static_cast<std::string>(name),
                            static_cast<CommOverlapType>(comm_type));
...
XLA_FFI_DEFINE_HANDLER_SYMBOL(CopyIntoOverlapBufferHandler, CopyIntoOverlapBufferFFI,
                              FFI::Bind()
                                  ...
                                  .Attr<int32_t>("comm_type"),
                              FFI_CudaGraph_Traits);

void CommGemmOverlapImpl(
    void *lhs, const std::vector<size_t> &lhs_shape, DType lhs_dtype, float *lhs_scale_inv,
    bool lhs_trans, void *rhs, const std::vector<size_t> &rhs_shape, DType rhs_dtype,
    float *rhs_scale_inv, bool rhs_trans, void *out, const std::vector<size_t> &out_shape,
    DType out_dtype, float *out_amax, float *out_scale, void *bias, DType bias_dtype,
    void *pre_gelu_out, void *extra_out, const std::vector<size_t> &extra_out_shape,
    void *workspace, size_t workspace_size, bool fuse_gelu, bool fuse_bias, bool grad,
    bool accumulate, bool use_split_accumulator, bool pipeline_rs_overlap_first_chunk,
  ...
  auto extra_out_ = TensorWrapper(extra_out, extra_out_shape, lhs_dtype, nullptr, nullptr,
                                   lhs_scale_inv);
  if (comm_type == CommOverlapType::AG) {
    // AG overlap is only ring-exchange
    auto overlap = reinterpret_cast<CommOverlapP2PBase *>(_overlaps[name]);
    ...
    if (_overlaps[name]->is_p2p_overlap()) {
      // RS overlap can be either ring-exchange or pipeline
      auto overlap = reinterpret_cast<CommOverlapP2PBase *>(_overlaps[name]);
      if (overlap->is_atomic_gemm()) {
        overlap->atomic_gemm_overlap_rs(rhs_, rhs_trans, lhs_, lhs_trans, out_, bias_,
                                        pre_gelu_out_, workspace_, grad, accumulate,
                                        use_split_accumulator, extra_out_, stream);
      } else {
      ...
      auto overlap = reinterpret_cast<CommOverlapBase *>(_overlaps[name]);
      if (overlap->is_atomic_gemm()) {
        overlap->atomic_gemm_overlap_rs(rhs_, rhs_trans, lhs_, lhs_trans, out_, bias_,
                                        pre_gelu_out_, workspace_, grad, accumulate,
                                        use_split_accumulator, pipeline_rs_overlap_first_chunk,
                                        extra_out_, stream);
      } else {
        overlap->split_overlap_rs(rhs_, rhs_trans, lhs_, lhs_trans, out_, bias_, pre_gelu_out_,
                                  workspace_, grad, accumulate, use_split_accumulator,
                                  pipeline_rs_overlap_first_chunk, extra_out_, stream);
      }
    }
  }
...
void CommGemmOverlap(cudaStream_t stream, void **buffers, const char *opaque, size_t opaque_len) {
  ...
  NVTE_CHECK(out_amax == out_amax_new,
  ...
  auto lhs_shape = (desc.lhs_trans) ? std::vector<size_t>{desc.k, desc.m}
                                    : std::vector<size_t>{desc.m, desc.k};
  auto rhs_shape = (desc.rhs_trans) ? std::vector<size_t>{desc.n, desc.k}
                                    : std::vector<size_t>{desc.k, desc.n};
  auto out_shape = std::vector<size_t>{desc.m, desc.n};

  CommGemmOverlapImpl(
      lhs, lhs_shape, desc.operand_dtype, lhs_scale_inv, desc.lhs_trans, rhs, rhs_shape,
      desc.operand_dtype, rhs_scale_inv, desc.rhs_trans, out, out_shape, desc.out_dtype, out_amax,
      out_scale, bias, desc.bias_dtype, pre_gelu_out, extra_out, lhs_shape, workspace,
      desc.workspace_size, desc.fuse_gelu, desc.fuse_bias, desc.grad, desc.accumulate,
      desc.use_split_accumulator, desc.pipeline_rs_overlap_first_chunk, desc.comm_type,
      desc.name, stream);
}

Error_Type CommGemmOverlapFFI(
    cudaStream_t stream, Buffer_Type lhs, Buffer_Type lhs_scale_inv, Buffer_Type rhs,
    Buffer_Type rhs_scale_inv, Buffer_Type bias, Buffer_Type gelu_input, Buffer_Type out_amax,
    Buffer_Type out_scale, Result_Type out, Result_Type out_amax_new, Result_Type out_scale_new,
    Result_Type pre_gelu_out, Result_Type bias_grad, Result_Type lhs_copy, Result_Type workspace,
    bool lhs_trans, bool rhs_trans, bool fuse_gelu, bool fuse_bias, bool grad, bool accumulate,
    bool use_split_accumulator, bool pipeline_rs_overlap_first_chunk, int32_t comm_type,
  ...
  auto lhs_copy_ptr = lhs_copy->untyped_data();
  ...
  CommGemmOverlapImpl(
      ...
      bias_ptr, bias_dtype, pre_gelu_ptr, lhs_copy_ptr, lhs_shape, workspace_ptr, workspace_size,
      fuse_gelu, fuse_bias, grad, accumulate, use_split_accumulator,
      pipeline_rs_overlap_first_chunk, static_cast<CommOverlapType>(comm_type),
      static_cast<std::string>(name), stream);
...
XLA_FFI_DEFINE_HANDLER_SYMBOL(CommGemmOverlapHandler, CommGemmOverlapFFI,
                              FFI::Bind()
                                  ...
                                  .Ret<Buffer_Type>()      // all-gathered lhs
                                  ...
                                  .Attr<bool>("pipeline_rs_overlap_first_chunk")
                                  .Attr<int32_t>("comm_type")
                                  .Attr<std::string_view>("name"),
                              FFI_CudaGraph_Traits);
