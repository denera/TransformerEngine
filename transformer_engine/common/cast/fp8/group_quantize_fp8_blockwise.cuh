...
constexpr int kMinBlocksPerSM1D = 6;
...
#pragma unroll 1
...

  {
    ...
    const unsigned src_lane =
        (threadIdx.x % THREADS_PER_WARP) / kNumThreadsLoad * kNumThreadsLoad;
    const unsigned mask = ((1u << kNumThreadsLoad) - 1u) << src_lane;
    const bool is_src_lane = (threadIdx.x % kNumThreadsLoad) == 0;
#pragma unroll
...
#pragma unroll
        for (int i = 0; i < kNVecIn; ++i) {
          ...
          amax = fmaxf(amax, fabsf(static_cast<float>(input_vec.input_type.data.elt[i])));
        }
#pragma unroll
        for (int delta = kNumThreadsLoad / 2; delta > 0; delta /= 2) {
...
#pragma unroll
          ...
          if (align_elts != 0) {
            ...
            if constexpr (!RETURN_ROWWISE) {
              ...
              const int prefix_vec_start = static_cast<int>(
                  (kNVecCleanup - (column_output_offset % kNVecCleanup)) % kNVecCleanup);
...
#pragma unroll
                for (int i = 0; i < kNVecCleanup; ++i) {
                  const IType value = smem[(cleanup_r + i) * kSMemCol + c].data.elt[smem_idx];
                  output_vec.data.elt[i] =
                      static_cast<OType>(static_cast<float>(value) * scale);
                }
              ...
              const int suffix_count = kTileDim - suffix_begin;
              const size_t suffix_output_offset = column_output_offset + suffix_begin;
              const int suffix_vec_start = static_cast<int>(
                  (kNVecCleanup - (suffix_output_offset % kNVecCleanup)) % kNVecCleanup);
              const int suffix_vec_count =
...
#pragma unroll
              for (int cleanup_idx = 0; cleanup_idx < kNVecOut / kNumThreadsStore;
                ...
                if (cleanup_r < align_elts) {
                  covered_by_vec =
                      cleanup_r >= prefix_vec_start &&
                      cleanup_r < prefix_vec_start + prefix_vec_count * kNVecCleanup;
                } else {
                  const int suffix_r = cleanup_r - suffix_begin;
                  covered_by_vec =
                      suffix_r >= suffix_vec_start &&
...
void group_quantize(const GroupedTensor *input, const Tensor *noop, GroupedTensor *output,
  ...
  // exact tile count. This avoids empty CTAs for jagged groups without changing split metadata.
  const bool compact_jagged_tiles =
      launch_first_dims_ptr != nullptr && configured_total_tensor_row_tiles != 0 &&
      configured_total_tensor_row_tiles < rectangular_row_tiles;
  const bool compact_1d_jagged_tiles = !IS_2D && compact_jagged_tiles;
  const bool compact_2d_jagged_tiles = IS_2D && compact_jagged_tiles;
          ...
          if constexpr (IS_2D) {
            ...
            if (output->has_data() && output->has_columnwise_data()) {
              ...
              if (compact_2d_jagged_tiles) {
                square_2d_tuned_kernel<IType, OType, true, true, false, true>
                    <<<grid, THREADS_PER_BLOCK_2D, 0, stream>>>(
                    reinterpret_cast<const IType *>(input->data.dptr),
                    reinterpret_cast<OType *>(output->data.dptr),
                    reinterpret_cast<OType *>(output->columnwise_data.dptr),
              ...
              if (compact_2d_jagged_tiles) {
                square_2d_tuned_kernel<IType, OType, true, false, false, true>
                    <<<grid, THREADS_PER_BLOCK_2D, 0, stream>>>(
                        reinterpret_cast<const IType *>(input->data.dptr),
...
#endif
              if (compact_2d_jagged_tiles) {
                square_2d_tuned_kernel<IType, OType, false, true, false, true>
                    <<<grid, THREADS_PER_BLOCK_2D, 0, stream>>>(
                        reinterpret_cast<const IType *>(input->data.dptr), nullptr,
                        reinterpret_cast<OType *>(output->columnwise_data.dptr), nullptr,
                        reinterpret_cast<float *>(output->columnwise_scale_inv.dptr), num_tensors,
            ...
            if (output->has_data() && output->has_columnwise_data()) {
              if (compact_1d_jagged_tiles) {
                vector_1d_tuned_kernel<kMinBlocksPerSM1DBoth, false, IType, OType, true, true,
                                        true>
                ...
                vector_1d_tuned_kernel<kMinBlocksPerSM1DBoth, true, IType, OType, true, true>
                    <<<grid, THREADS_PER_BLOCK_2D, smem_bytes, stream>>>(
                        reinterpret_cast<const IType *>(input->data.dptr),
                        reinterpret_cast<OType *>(output->data.dptr),
                        reinterpret_cast<OType *>(output->columnwise_data.dptr),
