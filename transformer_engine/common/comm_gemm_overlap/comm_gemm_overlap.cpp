...
void CommOverlapBase::copy_into_buffer(const TensorWrapper &input, bool local_chunk,
...
void CommOverlapP2PBase::copy_into_buffer(const TensorWrapper &input, bool local_chunk,
                                          cudaStream_t stream_main) {
  // Check element size
  const size_t element_size = input.element_size();
  NVTE_CHECK(_ubuf.element_size() == element_size,
