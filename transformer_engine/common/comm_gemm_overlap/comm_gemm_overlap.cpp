...
CommGemmOverlapBase::CommGemmOverlapBase(
    ...
    std::function<void(void *, size_t, int64_t, char *)> bcast_handle,
  ...
  if (!_comm_created) {
    ...
    if (backend == NVTE_Comm_Overlap_Backend::NVSHMEM) {
      cublasmplite::broadcast_fun_type broadcast = [&](void* data, size_t bytes, int root, int num_ranks) {
        ...
        bcast_handle(data, bytes, (int64_t)root, world);
      };
      ...
      _nvshmem_sync = getenv<bool>("NVTE_NVSHMEM_SYNC", 0);
      _nvshmem_p2p = cublasmplite::nvshmem_pipelined_p2p_t::create(worldrank, worldsize, broadcast, worldsize, signal, wait);
      if (worldrank == 0) {
        printf("[CommGemmOverlap] NVSHMEM communicator initialized, sync %d, signal %d, wait %d\n", (int)_nvshmem_sync, (int)signal, (int)wait);
      }
    } else {
...
CommGemmOverlap::CommGemmOverlap(
    ...
    : CommGemmOverlapBase(worldrank, worldsize, localrank, localsize, nodeid, numnodes, tp_size,
                          ...
                          use_ce, atomic_gemm, "CommOverlap", backend, allgather_handle, bcast_handle,
                          barrier_handle) {
  if (_atomic_gemm) {
...
CommGemmOverlapP2P::CommGemmOverlapP2P(
    ...
    : CommGemmOverlapBase(worldrank, worldsize, localrank, localsize, nodeid, numnodes, tp_size,
                          ...
                          atomic_gemm, "CommOverlapP2P", backend, allgather_handle, bcast_handle,
                          barrier_handle) {
  _is_p2p = true;
