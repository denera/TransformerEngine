...
#define GPU_PAGE_MASK (~GPU_PAGE_OFFSET)
  ...
  NVTE_CHECK_CUDA(cudaMalloc(reinterpret_cast<void **>(&(*comm)->flags), 2 * GPU_PAGE_SIZE));
...
void destroy_communicator(communicator *comm) {
  ...
  for (int hndl = 0; hndl < comm->free_region; hndl++) {
    if (comm->use_mc && comm->mem_dealloc[hndl]) {
      ...
      // Unmap memory addresses for both peer and own buffers
      ...
      // Release local memory for buffer allocation handles
      NVTE_CALL_CHECK_CUDA_DRIVER(cuMemRelease, comm->uchandles[hndl][comm->nvrank]);
  ...
  NVTE_CHECK_CUDA(cudaFree(reinterpret_cast<void *>(comm->flags)));
  if (comm->use_mc) {
    NVTE_CALL_CHECK_CUDA_DRIVER(cuMemAddressFree, comm->mc_baseptr, comm->mc_maxsize);
    NVTE_CALL_CHECK_CUDA_DRIVER(cuMemUnmap,
        reinterpret_cast<CUdeviceptr>(comm->mc_handle), comm->mc_maxsize);
