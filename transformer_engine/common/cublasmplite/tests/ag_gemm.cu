...
int test(const size_t m, const size_t n, const size_t k, 
         const nvshmem_pipelined_p2p_t::signal_kind signal, const nvshmem_pipelined_p2p_t::wait_kind wait,
         int comm_sms, const size_t cycles, const size_t skip, const bool verbose, const bool csv, MPI_Comm mpi_comm) {

    // MPI
    const mpi_t mpi(mpi_comm, !csv);

    if(mpi.my_rank == 0 && !csv) {
        printf("AG+GEMM: num_ranks %d | m x n x k = %zu x %zu x %zu | cycles %zu | skip %zu | signal %d | wait %d\n", mpi.num_ranks, m, n, k, cycles, skip, (int)signal, (int)wait);
    }
    ...
    if(mpi.my_rank == 0 && csv) {
        printf("<<<<, num_ranks, m, n, k, signal, wait, comm_sms, cycles, skip, sizeof(T), NVSHMEM time [ms], NVSHMEM perf [GFlop/s], NCCL time [ms], NCCL perf [GFlop/s], L2 relative error\n");
        printf(">>>>, %d, %zu, %zu, %zu, %d, %d, %d, %zu, %zu, %zu, %e, %e, %e, %e, %e\n", 
            mpi.num_ranks, m, n, k, (int)signal, (int)wait, comm_sms, cycles, skip, sizeof(T), 
...
int main(int argc, char** argv) {
    ...
    options.add_options()
        ...
        ("signal", "signal for NVSHMEM opt (set or add)", cxxopts::value<std::string>()->default_value("set"))
        ("wait", "wait on signal mechanism (nvshmem_wait or cu_stream_wait)", cxxopts::value<std::string>()->default_value("cu_stream_wait"))
    ...
    const std::string wait = result["wait"].as<std::string>();

    CUBLASMPLITE_ASSERT(signal == "set" || signal == "add");
    CUBLASMPLITE_ASSERT(wait == "nvshmem_wait" || wait == "cu_stream_wait");
    ...
    nvshmem_pipelined_p2p_t::wait_kind w = (wait == "nvshmem_wait") ? nvshmem_pipelined_p2p_t::wait_kind::nvshmem_wait : nvshmem_pipelined_p2p_t::wait_kind::cu_stream_wait;

    MPI_CHECK(MPI_Init(nullptr, nullptr));

    int error = test(m, n, k, s, w, comm_sms, cycles, skip, verbose, csv, MPI_COMM_WORLD);
